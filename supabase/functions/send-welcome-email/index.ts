...
import { isWelcomeEligible, welcomeCutoffIso } from "./welcomeEligibility.ts";
...
const accepted = (headers: Record<string, string>) => new Response(
...
serve(async (req: Request): Promise<Response> => {
  ...
  if (!supabase || !supabaseUrl || !resendApiKey || !unsubscribeSecret || !newsletterPostalAddress) {
  ...
  try {
    ...
    const { data: subscriber, error: lookupError } = await supabase
      .from("subscribers")
      .select("id, subscribed, created_at, welcome_email_sent_at")
      .eq("email", email)
      .gte("created_at", welcomeCutoffIso())
      .maybeSingle();

    if (lookupError || !isWelcomeEligible(subscriber)) {
      if (lookupError) {
        await logRun("failure", "welcome eligibility lookup failed", {}, startedAt);
      }
    ...
    if (!claimed) return accepted(headers);
    ...
    const response = await resend.emails.send({
      ...
      html: `
        <div style="font-family:Inter,Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px 22px;color:#17233a">
          ...
          <p style="margin-top:34px;padding-top:18px;border-top:1px solid #e5e7eb;font-size:12px;line-height:1.6;color:#64748b">You received this because you subscribed at churnisdead.com.<br>Churn Is Dead, ${escapeHtml(newsletterPostalAddress)}<br><a href="${unsubscribeUrl}" style="color:#64748b;text-decoration:underline">Unsubscribe in one click</a>.</p>
        </div>`,
    });
