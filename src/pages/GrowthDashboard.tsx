...
type VariantRow = { source: string; medium: string; campaign: string; variant: string; visits: number; signups: number; active_subscribers: number };
type DashboardData = {
...
const VariantTable = ({ rows }: { rows: VariantRow[] }) => (
  <section className="rounded-xl border border-gray-200 bg-white p-6">
    ...
    <p className="mb-5 text-xs text-gray-500">Acquired signups stay in the campaign history. “Still active” shows how many remain subscribed now.</p>
    <div className="overflow-x-auto">
      <table className="w-full min-w-[640px] text-left text-sm">
        <thead className="border-b border-gray-200 text-[10px] uppercase tracking-[0.16em] text-gray-500">
          <tr><th className="pb-3 pr-4">Channel</th><th className="pb-3 pr-4">Campaign</th><th className="pb-3 pr-4">Variant</th><th className="pb-3 pr-4 text-right">Visits</th><th className="pb-3 pr-4 text-right">Acquired</th><th className="pb-3 pr-4 text-right">Still active</th><th className="pb-3 text-right">Visit → signup</th></tr>
        </thead>
        <tbody>
          {rows.map(row => {
          ...
          {rows.length === 0 && <tr><td colSpan={7} className="py-5 text-gray-500">No tagged variants yet.</td></tr>}
        </tbody>
      </table>
    </div>
  </section>
