// Analysis sidebar component with grouping support
...
export const AnalysisSidebar = ({
  ...
  const handleSelect = (analysis: Analysis) => {
    ...
    // Also set the group context when selecting an analysis
    if (analysis.group_id) {
      onSelectGroup?.(analysis.group_id);
      setExpandedGroups((prev) => new Set(prev).add(analysis.group_id!));
    } else {
      onSelectGroup?.(null);
    }
  };
  ...
  const renderAnalysisItem = (analysis: Analysis, indented = false) => (
    <div
      key={analysis.id}
        ...
        {!user ? (
          ...
          <div className={cn("px-2 py-1 space-y-0.5", isCollapsed && "px-2")}>
              ...
              if (isCollapsed) {
                return (
                  <div
                    key={group.id}
                    className={cn(
                      "p-2 flex justify-center rounded-md cursor-pointer",
                      isSelected ? "bg-white/15" : "hover:bg-white/10"
                    )}
                    title={`${group.name} (${groupAnalyses.length})`}
              ...
              return (
                ...
                >
                    ...
                    >
                      {isExpanded ? (
                        <>
                          ...
                          <FolderOpen className="h-4 w-4 text-white/50 shrink-0" />
                        </>
                      ) : (
                        <>
                      ...
                      <span className="flex-1 text-left truncate text-white/80 font-medium">
                        {group.name}
                      </span>
                  ...
                  <CollapsibleContent className="space-y-0.5 mt-0.5">
                    {groupAnalyses.map((analysis) => renderAnalysisItem(analysis, true))}
                  </CollapsibleContent>
                </Collapsible>
              );
            })}
            ...
            {ungroupedAnalyses.length > 0 && (
              <div className="mt-1">
                {!isCollapsed && groups.some((g) => (groupedAnalyses[g.id] || []).length > 0) && (
                  <div className="px-3 py-1 text-xs font-medium text-white/40 uppercase tracking-wide">
                    Ungrouped
                  </div>
                )}
                <div className="space-y-0.5">
                  {ungroupedAnalyses.map((analysis) => renderAnalysisItem(analysis))}
                </div>
              </div>
            )}
          </div>
