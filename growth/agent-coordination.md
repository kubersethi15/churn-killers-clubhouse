...
Reach 15,000 relevant LinkedIn followers by 31 December 2026 while increasing
...
|---|---|---|---|---|
| SITE-04 QBR template continuity repair | Codex | merged PR [#187](https://github.com/kubersethi15/churn-killers-clubhouse/pull/187) | Complete 3 September 2026 | Replaced the incorrect six-page guide and generic four-question workspace with the confirmed thirty-minute, three-slide template. The exact LinkedIn query isolates one result, the dedicated page contains zero questionnaire fields, the stable PDF path returns the visually verified two-page asset, and desktop/mobile QA, TypeScript, lint, production build and 19 public-route checks passed. No files remain claimed. |
| LI-15K controllable follower growth | Codex + social manager | `growth/linkedin-15k-operating-plan.md`; `growth/linkedin-creative-intelligence-system.md`; shared `Growth Scorecard`, `15K Creative Lab` and `Engagement List`; active growth heartbeat | Active through 31 December 2026 | Baseline 8,363 on 28 August; target 15,000. The 4 September read closed at 8,503 followers, 229 below the first target line; seven-day reach was 2,156 with 43% out of network, while the 1 and 3 September posts produced eight profile viewers and one attributed follow from 1,804 members reached. Reach is improving but conversion is weak. Kuber has chosen no paid promotion at this stage. Final 6 September QA locked the 7–12 September block in shared-calendar rows 247–252: human APJC AI-leadership field note, Churn Is Dead ownership tool, native LinkedIn Newsletter, genuine product-versus-relationship belief change, saveable AI decision document and short human CS observation. A live creator-topic collision was removed rather than copied. Codex owns up to ten de-duplicated qualified weekday external comments across three daytime windows plus aggregate reads; the manager owns the approved calendar execution, Monday photo, Friday document rendering, evening engagement and response coverage. Monday's chosen natural photo and Friday's final rendered preview must be checked before scheduling; Wednesday remains the native Newsletter only. Redesign up to three following-week jobs if the block still produces no meaningful profile/follow movement; never add generic volume or duplicate comments. |
...
| CID-004 LinkedIn Newsletter reactivation | Codex + social manager | Existing `Churn is Dead` LinkedIn Newsletter | First edition live 26 August; remaining editions scheduled and verified for 2, 9 and 16 September at 18:15 Sydney | The first edition is live at `https://www.linkedin.com/pulse/product-does-need-another-customer-request-kuber-sethi-njhkc/` with the exact tracked canonical link and no duplicate Wednesday post. Preserve `linkedin/newsletter/linkedin_newsletter` attribution and run the aggregate CID-004 report at 24 hours and after each seven-day window; do not change cadence below the four-edition evidence floor. |
...
| DELIV-04 approved subscriber-email packages | Codex | `editorial/renewal-email-release-package`; three approved upcoming issue packages and the editorial validator | Renewal Evidence Packet partially delivered; safe-resume audit required before any further invocation | The 4 September privacy-safe reconciliation found the approved subject already in Resend. The provider now shows 524 rolling-window emails, 99.43% deliverability, 0.57% bounce (three transient, zero permanent) and zero complaints. Webhook events record 197 sent and 197 delivered messages, while the send ledger records 197 sent rows and two failures. Of 320 currently active subscribers, 189 are logged sent, 129 are unsent and two are logged failed. Do not run a full duplicate broadcast. Identify why the prior run stopped, then resume only the unsent active set through the existing idempotent ledger and recheck aggregate outcomes. |
| BC-09 six-week growth scorecard | Codex | shared Google Sheet `Growth Scorecard` tab; active growth heartbeat | Active 26 August through 4 October 2026 | One aggregate row per week joins active subscribers, confirmed acquisition, 30-day retention, clean sessions, channel-labelled visits-to-subscriber outcomes, qualified conversations and email health. Friday reviews apply the published evidence gates and record one `KEEP`, `IMPROVE`, `PIVOT` or `PAUSE` decision. List cleanup is never counted as failed acquisition; no channel winner is declared below its evidence floor. Claude may use the scorecard evidence but must not create a duplicate dashboard or weekly review. |
