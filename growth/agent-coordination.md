...
| DELIV-03 subscriber email release quality | Codex | merged PR [#152](https://github.com/kubersethi15/churn-killers-clubhouse/pull/152); Resend domain configuration | Code and issue metadata complete; production deployment/test held 25 August | Sender, template, tests, current subject/preheader, and release audit are merged. Resend open and click tracking are off, preserving direct site links and on-site UTM attribution. Codex retains the deploy/test boundary until Kuber supplies the lawful mailing address and completes secure Supabase CLI login. Do not enable `NEWSLETTER_SEND_ENABLED` or send to subscribers from this lane. |
