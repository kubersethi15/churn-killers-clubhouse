...
| DELIV-03 subscriber email release quality | Codex | `email/best-in-class-subscriber-release` | Active 25 August; production broadcast remains disabled | Claimed: `send-latest-newsletter` template/sender/tests, current issue subject and preheader metadata, and email release documentation. Scope is one faithful plain-text alternative, a single clear website route, production-equivalent test headers/content, subject/preheader quality, mobile rendering, and a written deliverability gate. Do not enable `NEWSLETTER_SEND_ENABLED` or send to subscribers from this lane. |
