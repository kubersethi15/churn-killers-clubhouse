...
| LI-15K controllable follower growth | Codex + social manager | `growth/linkedin-15k-operating-plan.md`; `growth/linkedin-creative-intelligence-system.md`; shared `Growth Scorecard`, `15K Creative Lab` and `Engagement List`; active growth heartbeat | Active through 31 December 2026 | Baseline 8,363 on 28 August; target 15,000; required pace about 370 net relevant followers per week. This supersedes the 20K target. From 31 August, Codex owns a ten-weekday test of up to ten qualified external comments across three daytime Sydney windows; exact batches still require action-time confirmation, missed capacity is never dumped later, and every URL is logged. The social manager retains the separate evening engagement, approved calendar execution, exact visual production and response coverage; both operators must de-duplicate against the shared `Engagement List`. Preserve approved public work through the 1 September CID-001 close, then run the follower-funnel, opening, native-asset and timing tests one variable at a time. The next five comparable feed posts use two evolved Kuber winners, one external-pattern adaptation, one original format bet and one Churn Is Dead issue/tool post without adding volume. Codex owns aggregate reads, the creative scan and one weekly portfolio decision. Claude may claim one non-duplicative evidence-backed format research brief only after checking this row and must not create a parallel scorecard or duplicate comments. |
...
| DELIV-04 approved subscriber-email packages | Codex | `editorial/renewal-email-release-package`; three approved upcoming issue packages and the editorial validator | Renewal Evidence Packet held on 2 September because Resend still marks sender health `RISK`; production switch confirmed off | Selected one direct subject and controlled preheader for each approved 1, 8 and 15 September issue instead of splitting roughly 312 active readers across three underpowered variants. The 2 September recovery read shows 334 rolling-window emails, 94.31% deliverability, 19 transient bounces, zero permanent bounces and zero complaints. Eleven provider bounce suppressions remain in place and no production send exists for the approved subject. Keep the exact approved package ready, but do not suppress temporary failures or enable production until Resend's current risk classification clears and every normal gate passes. |
