...
| DELIV-02 broadcast retry idempotency | Claude | branch `growth/broadcast-retry-idempotency`; PR [#144](https://github.com/kubersethi15/churn-killers-clubhouse/pull/144) | PR open; focused tests pass | Claimed: `supabase/functions/send-latest-newsletter/index.ts`, new `sendPlan.{ts,test.ts}`, and migration `20260901010000_dedupe_newsletter_send_log.sql`. Recipient-idempotent under partial-send retries and subscriber-list changes: the primary guard pre-filters already-delivered recipients from `newsletter_send_log` before batching (aborts if that log is unreadable), so a reshuffled batch cannot re-send. Ordered recipients and content-addressed batch keys are a within-run backstop only. Send-log upsert on a new unique index; the pointer advances only when no batch transiently failed AND the log persisted. No broadcast enabled; NEWSLETTER_SEND_ENABLED untouched; no launch/editorial surface. Separate from DELIV-01 (#143). |
