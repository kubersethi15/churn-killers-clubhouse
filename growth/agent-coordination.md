...
| DELIV-01 unsubscribe-token contract hardening | Claude | branch `growth/deliverability-unsubscribe-token-contract` | PR open; focused tests pass | Claimed: `supabase/functions/unsubscribe-newsletter/index.ts` and `supabase/functions/send-latest-newsletter/unsubscribeToken.{ts,test.ts}`. Deduped the hand-copied verifier so unsubscribe now imports the shared signer module, and locked the token contract with a golden-vector test. Behaviour-preserving; no schema change, no broadcast enabled, no launch surface touched. Do not re-inline a second verifier copy. |
