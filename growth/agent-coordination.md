...
| CG-15/ACQ-06 curated-directory + adjacent-swap routes | Claude | `growth/cg15-gtmindex-customer-education-routes` | Prepared and staged 26 August; nothing sent | Claimed: the new rows in `growth/outreach-pitches.md`, `growth/cross-newsletter-collaboration.md`, `growth/channel-pipeline.csv` (rows 25/26), the action-log entry, and this row. Two verified non-overlapping routes: The GTM Index CS directory (row 25) and Customer Education Bi-Weekly swap (row 26). Does not touch Codex's Wednesday newsletter, Resend, funnel measurement, LinkedIn comments, CG-18 Medium, the Reddit PR #105 lane, or the RevOps Impact swap. Codex or Kuber confirms each first-party route and sends. |
...
(both routes plus the CS Cafe rejection), `growth/channel-pipeline.csv` (rows 25
and 26), `growth/action-log.md`, and this file. **Nothing was sent; no external
...
swap, with one competitor swap rejected. It is deliberately non-overlapping: it
...
parked Reddit contribution (PR #105) or the RevOps Impact swap.
...
- **Owner:** Codex or Kuber submits after confirming the exact first-party route
  (site `/about` contact or Rome Thorndike on LinkedIn). No address invented. Copy
  in `growth/outreach-pitches.md`.
...
A swap with **The CS Cafe** (Hakan Ozturk, 4,300+ core-CS) is low value: it is a
direct competitor and would not route readers to a rival. The ChurnZero, Sybill,
TheCXLead, and TopCSJobs roundups are vendor or SEO content marketing rather than
independent curation and were declined. None are staged.

**Next owner action:** confirm each first-party route and send routes 1 and 2;
then the next unblocked Claude item is one more verified non-competing swap
(Customer Education is now staged; SaaS Barometer needs author verification) or a
second independently-curated, no-pay CS directory.
