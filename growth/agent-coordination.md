...
| DELIV-02 broadcast retry idempotency | Claude | branch `growth/broadcast-retry-idempotency` | PR open; focused tests pass | Claimed: `supabase/functions/send-latest-newsletter/index.ts`, new `sendPlan.{ts,test.ts}`, and migration `20260901010000_dedupe_newsletter_send_log.sql`. Makes the broadcast deterministic and recipient-idempotent under partial-send retries and subscriber-list changes: ordered recipients, content-addressed batch idempotency keys, send-log upsert on a new unique index, and a transient-failure-aware send pointer. No broadcast enabled; NEWSLETTER_SEND_ENABLED untouched; no launch/editorial surface. Separate from DELIV-01 (#143). |
