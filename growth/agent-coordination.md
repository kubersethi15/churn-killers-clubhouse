...
| CG-09 costly-problem ledger and analytics guard | Claude | merged PR [#90](https://github.com/kubersethi15/churn-killers-clubhouse/pull/90); corrective package BC-01 required | Guard merged; ledger corrections and tests still required | Preserve the production-host guard; correct unique-session counting, genuinely independent source classes, `resource_id` mapping, tests, and contamination wording in a new focused PR. Manifest files remain unchanged. |
...
| SEO-08 crawlable homepage identity schema | Codex | `growth/seo-static-homepage-schema` | Active | Move the existing factual WebSite, Organization, and Kuber Person schema into the built root HTML only; test that it is valid and does not leak onto non-home routes. No launch, CTA, copy, or attribution change. |
